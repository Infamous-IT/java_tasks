...
import org.example.streamAPI.models.UserDB;
import org.example.streamAPI.models.UserDTO;
...
public class Main {
    public static List<UserDTO> userDBToUserDTO(final List<UserDB> users) {
        return users
                .stream()
                .map(userDB -> new UserDTO(userDB.getUsername(), userDB.getFirstName(), userDB.getLastName(), userDB.getCity(),
                        userDB.getCountry(), userDB.getBirthdayYear()))
                .toList();
    }
    ...
    public static Map<Character, Long> getCharsFrequencyFromLastName(final List<UserDB> users) {
        ...
        // similar .collect(Collectors.groupingBy(character -> character, Collectors.counting()));
    }
}